...
export const login: AsyncActionCreator<Action> = (data: AuthCredentials, service: AuthService, successCallback: () => void) => async dispatch => {
  dispatch(initProgress(ProgressType.AUTHENTICATING));
  try {
    await service.login(data);
    dispatch(stopProgress(ProgressType.AUTHENTICATING));
    dispatch({type: ActionType.AUTHENTICATION_SUCCESSFUL, token: 'Yzx34abdgaeorifornvjbawkeufbdcnksjdf788'});
...
export const register: AsyncActionCreator<Action> = (data: User, service: AuthService) => async dispatch => {
